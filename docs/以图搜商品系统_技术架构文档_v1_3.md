...
### 12.4 自动化测试矩阵（v1.6 新增）

覆盖 **65 个业务场景**, 62 个测试文件, **379 个测试用例**, 5 个微服务 + 跨服务 E2E 全覆盖。

**测试分层与运行方式**：

```bash
make test-all            # 全部单元测试 (无需外部依赖, CI 必跑)
make test-all-integ      # 全部集成 + E2E (需 docker-compose 基础设施)
```

**按服务覆盖矩阵**：

| 服务 | 测试文件 | 单元测试 | 集成测试 | E2E | 覆盖业务场景 |
|------|---------|---------|---------|-----|-------------|
| write-service | 15 | 74 | 6 | 2 | 两级去重、图片下载重试、特征提取 (推理/mock/异常)、常青分类、词表编码、Milvus upsert (热区/常青)、PG 补偿、Bitmap 推送 (gRPC+Kafka)、锁 watchdog、Kafka 事件、端点处理器 (update_image/bind_merchant/update_video)、全流程编排、flush 批次 |
| search-service | 27 | 202 | 8 | — | 8-Stage Pipeline、熔断器生命周期、5态降级 FSM、Bitmap 3级降级、scope 解析、3级词表缓存、4路融合排序、GPU→CPU 降级、Tag 召回、Float32 精排、级联/常青兜底、API 限流鉴权、Admin API (降级覆盖/熔断器控制)、行为上报、搜索日志异步发送、配置服务+审计、Pipeline 集成 (CB联动/降级参数) |
| cron-scheduler | 8 | 35 | 8 | 2 | 分区轮转 (18月截止/常青保护/补偿)、URI 去重清理+VACUUM、Bitmap 对账、常青池治理 (GREEN/YELLOW/RED)、Milvus compaction |
| flink-pipeline | 5 | 28 | 5 | — | Kafka 反序列化、窗口聚合 (ADD/REMOVE/合并)、PG Sink upsert、字典编码、MiniCluster 集成 |
| bitmap-filter-service | 5 | 35 | 5 | — | RocksDB 双列族 (cf_rolling/cf_evergreen)、CDC 事件路由+DLQ、并行 MultiGet+AND 过滤、健康检查+自动弹出/恢复 |
| 跨服务 E2E | 4 | — | — | 5 | 写入→搜索可见性、写入→Flink→Bitmap→搜索、分区轮转→搜索不中断、常青清理→搜索反映变化 |
| **合计** | **62** | **374** | **32** | **9** | **65 业务场景** |

**65 个业务场景分类**：

- **写入流程 (15)**: 去重、下载、特征提取、常青分类、词表编码、Milvus upsert、PG 记录、Redis 缓存、flush 管理、Bitmap 推送、Kafka 事件、分布式锁、补偿日志、写入指标、视频帧提取
- **搜索流程 (23)**: 8-Stage Pipeline、熔断器、降级 FSM (5态+8种转移)、Bitmap 3级降级、scope 解析、词表缓存、融合排序、特征提取、精排、Tag 召回、级联搜索、常青兜底、bitmap skip→S2、API 端点 (鉴权/限流/健康)、Admin API (降级覆盖/熔断器控制/配置重载/配置审计/限流器状态)、行为上报、搜索日志、配置服务
- **数据生命周期 (9)**: 分区轮转、URI 清理、VACUUM、Bitmap 对账、常青 GREEN/YELLOW/RED 治理、Milvus compaction、常青批量清理、补偿记录
- **Flink 流处理 (6)**: 反序列化、窗口聚合、PG Sink、字典编码、gRPC 推送、checkpoint 恢复
- **Bitmap 过滤 (8)**: RocksDB 双 CF、CDC 路由、DLQ 处理、并行 MultiGet、AND 过滤、健康检查、自动弹出/恢复、PG-RocksDB diff
- **跨服务 E2E (4)**: 写入→搜索、写入→Flink→Bitmap→搜索、轮转→搜索、常青→搜索
